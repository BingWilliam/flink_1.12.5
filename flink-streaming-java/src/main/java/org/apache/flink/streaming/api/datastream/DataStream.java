...
public class DataStream<T> {
    ...
    public <R> SingleOutputStreamOperator<R> flatMap(
        ...
        /***************************************************************************************
         * TODO flink 把每一个算子transform 成一个对流的转换
         *  并且注册到执行环境中，用于生成 StreamGraph
         *  -
         *  第一步：用户代码里定义的UDF 会被当作其基类对待，然后交给 StreamFlatMap 这个 Operator 做进一步包装
         *  事实上，每一个 Transformation 都对应一个 StreamOperator
         *  -
         *  flink 流式计算的核心概念，就是将数据从输入流一个一个传递给 Operator 进行链式处理，最后交给输出流的过程
         *  -
         *  StreamFlatMap 是一个 Function 也是一个 StreamOperator
         *  -
         *  StreamFlatMap = StreamOperator
         *  flatMapper = Function
         *  - 最终调用 transform 方法来把 StreamFlatMap 将StreamOperator 转换成 Transformation
         *  最终加入到 StreamExecutionEnvironment 的  List<Transformation<?>> transformations
         */
    ...
    protected <R> SingleOutputStreamOperator<R> doTransform(
        ...
        /***************************************************************************************
         * TODO  构建：OneInputTransformation
         *  由于 flatMap 这个操作只接受 一个输入，所以在进一步包装成为 OneInputTransformation
         */
