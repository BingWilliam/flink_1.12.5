...
public abstract class ResourceManager<WorkerType extends ResourceIDRetrievable>
    ...
    /***************************************************************************************
     * TODO
     *  执行完毕 这个构造 方法的时候，会触发调用 onStart() 方法执行
     */
    ...
    /***************************************************************************************
     * TODO
     *  如果执行到这，也就意味着：ResourceManager 的实例化已经完成了
     *  接下来执行初始化
     */
    @Override
    ...
    private void startResourceManagerServices() throws Exception {
        try {
           ...
           // TODO 在Standalone 模式下，什么也没有做
            initialize();
            // TODO 注意 这个this 对象
            // TODO 执行选举，成功之后，调用 leaderElectionService.isLeader()
    ...
    // TODO: 11/12/2021 ResourceManager 注册 TaskExecutor 处理注册
    @Override
    ...
    private RegistrationResponse registerTaskExecutorInternal(
        ...
        // TODO: 12/12/2021  获取 TaskExecutor 的 ResourceID
        ...
        if (oldRegistration != null) {
            ...
            // TODO: 12/12/2021 从 SlotManager 中移出旧的 TaskManager
        ...
        if (newWorker == null) {
             ...
             *  1、主节点启动注册服务
             *  2、从节点启动
             *  3、先封装自己的信息，通过RPC 发送给主节点
             *  4、主节点处理注册，其实就是给这个从节点分配一个全局唯一的ID(从节点启动的时候就已经生成好了)
             *     再通过 ID -----> Registration 注册对象，这种映射关系保持在 主节点的内存中。
             *  5、如果是 HBase 除了 regionServer 上线之后向Master 注册外，还会把自己的信息写到 ZK 上
             *
             */


            /***************************************************************************************
             * TODO 维持心跳 = monitorTarget
             *  主节点主动
            ...
            taskManagerHeartbeatManager.monitorTarget(
                    ...
                    new HeartbeatTarget<Void>() {
                        ...
                        public void requestHeartbeat(ResourceID resourceID, Void payload) {
                            /***************************************************************************************
                             * TODO 发送心跳 RPC 请求 回到 TaskExecutor
                             */
    ...
    private CompletableFuture<Boolean> tryAcceptLeadership(final UUID newLeaderSessionID) {
        /***************************************************************************************
         * TODO
         *   判断，如果集群有了 LeaderResourceManager
         */
        if (leaderElectionService.hasLeadership(newLeaderSessionID)) {
            // TODO 申请一个 ResourceManagerID
            ...
            // TODO 如果之前已成为 Leader 的话，则清理之前的状态
    ...
    /***************************************************************************************
     * TODO
     *  当前ResourceManager 启动了两个心跳服务
     *   taskManagerHeartbeatManager 关心点：TaskManager
     *   jobManagerHeartbeatManager 关心点：jobManager
     *   taskManager 集群的资源提供者，任务执行者，从节点
     *   jobManager 每一个job 会启动的一个主控程序
     *   不管是集群的从节点执行心跳，还是每一个 job 会启动一个主控程序 都是向 ResourceManager 去汇报
     */
