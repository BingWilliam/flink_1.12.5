...
public class DefaultLeaderElectionService
    ...
    /***************************************************************************************
     * TODO 注意这个方法，因为当前这个类是 LeaderLatchListener 的子类，所以当该组件在进行选举如果成功的话
     *  则会自动调用 isLeader() 方法，否则调用 notLeader 方法
     *  这是 zookeeper 的API 框架 cruator 机制
     */
    @Override
    public final void start(LeaderContender contender) throws Exception {
        ...
        synchronized (lock) {
            ...
            // 选举完 执行 在这里是 执行 ZookeeperLeaderElectionDriver 的isLeader()
            running = true;
        }
    }
    ...
    public void onGrantLeadership() {
        synchronized (lock) {
            if (running) {
                 ...
                 *  leaderContender = ResourceManger
