...
public class WebMonitorEndpoint<T extends RestfulGateway> extends RestServerEndpoint
     ...
     *  不管这里初始化哪个 handler ,里面都有一个 handleRequest() 方法
     *  channel0() 的底层，最终调用的就是 Handler.handleRequest() 方法
     *  将来我们提交job 的时候，最终，由WebMonitorEndPoint 接受到，跳转到使用 JobSubmitHandler 来执行
     *  最终执行请求 handlerRequest()
     */
    @Override
    protected List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(
        ...
        /***************************************************************************************
         * TODO
         *  这些 Handler 的作用，其实就是对应到 Flink web 业务的 rest 服务 ，handler = serverlet
         *  bigdata01:port/list
         */
        return handlers;
    }
    ...
    public void startInternal() throws Exception {
         ...
         *   不管在哪有：leaderElectionService.start(this)
         *    1、参与选举的某个获胜的角色会调用：ZooKeeperLeaderElectionDriver.isLeader()  ==> leaderElectionService.onGrantLeadership()
         *    2、参与选举的某个获胜的角色会调用：ZooKeeperLeaderElectionDriver.noLeader()  ==> leaderElectionService.onRevokeLeadership()
         */
    ...
    private void startExecutionGraphCacheCleanupTask() {
        ...
        /***************************************************************************************
         * TODO 开启定时任务：executionGraphCache.cleanup() 清除已经执行完的 ExecutorGraph
    ...
    public void grantLeadership(final UUID leaderSessionID) {
        ...
        /***************************************************************************************
         * TODO
         *  至此 WebMonitorEndpoint 启动好了，并没有做什么
         */
    }
