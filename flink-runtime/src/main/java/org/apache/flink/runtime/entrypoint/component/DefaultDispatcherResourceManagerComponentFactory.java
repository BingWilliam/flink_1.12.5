...
public class DefaultDispatcherResourceManagerComponentFactory
    ...
    public DispatcherResourceManagerComponent create(
        ...
        try {
            ...
            /***************************************************************************************
             * TODO 创建 WebMonitorEndPoint 实例，在 Standalone 模式下：DispatcherRestEndpoint
             *  1、restEndpointFactory = SessionRestEndPointFactory
             *  2、webMonitorEndpoint = DispatcherRestEndpoint
             *  3、highAvailabilityServices.getClusterRestEndpointLeaderElectionService()
             *     = zookeeperLeaderElectionSession
             *  接收Flink WEB 端的请求
             */
            webMonitorEndpoint =
            ...
            /***************************************************************************************
             * TODO 启动DispatcherRestEndPoint
             *  1、启动Netty 服务端
             *  2、选举
             *  3、启动定时任务 ExecutorGraphCacheCleanupTask
             */
            ...
            /***************************************************************************************
             * TODO 创建 StandaloneResourceManager
            ...
            /***************************************************************************************
             * TODO 创建并 启动 Dispatcher
             *  1、dispatcherRunner = DispatcherRunnerLeaderElectionLifecycleManager
             *  2、dispatcherRunner = DefaultDispatcherRunnerFactory
             *  第一参数为：ZookeeperLeaderElectionService
             *
             *  老版本：这个地方直接创建一个Dispatcher 对象
             *  然后调用 dispatcher.start() 来启动。
             *  新版本：直接创建 DispatcherRunner 内部就是创建和启动 Dispatcher
             */
            dispatcherRunner =
